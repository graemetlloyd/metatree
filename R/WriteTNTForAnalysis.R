...
WriteTNTForAnalysis <- function(clad.matrix, file.name) {
    ...
    if(ntaxa <= 24) { # If there are few enough taxa for an exact solution
        ...
        anal <- c(anal, c("hold 100000;", "shortread scratch.tre;", "bbreak=tbr;")) # Read in new technology trees and finish with a heuristic (tbr) search for all mpts
    }
    ...
    tnt.file <- c("mxram 4096;", toplines, mtrx, ";", ccode, anal, "nelsen*;", strict.name, mrp.name, "proc/;")
